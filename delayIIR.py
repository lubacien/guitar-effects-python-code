...
#delay parameters:
delaygain = 0.6
delaylen = int(0.16*32000) #must be less than half_buffer_len
...
def process(input_buffer, output_buffer, buffer_len, case ):
    ...
    if(case == True):
        ...
        for n in range(0,buffer_len):
            if n >= delaylen:
                output_buffer[n] = input_buffer[n]/2 + delaygain * output_buffer[n-delaylen]
            else:
                output_buffer[n] = input_buffer[n]/2 + delaygain * output_buffer[n - delaylen + full_buffer_len]
    else:
        # process one sample at a time
        for n in range(buffer_len,2*buffer_len):
            output_buffer[n] = input_buffer[n] + delaygain * output_buffer[n-delaylen]
...
# write to WAV
wavfile.write("delayIIR.wav", samp_freq, signal_proc)